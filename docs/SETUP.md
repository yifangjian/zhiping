...
機器人預設不預設任何角色——它從零開始認識使用者。要調整成你自己的用途,大多只需要動一兩個地方。

### 改背景(最常改的)

**預設是空的**,也就是機器人一開始完全不認識使用者,由對話慢慢認識。這是刻意的:預設一份背景等於替使用者決定他是什麼樣的人,而且猜錯的話,它從第一句話就會講得很怪。

如果你**很清楚**要陪伴的對象是誰,想省掉冷啟動,可以打開 [`app/prompts.py`](../app/prompts.py) 填 `DEFAULT_PROFILE`:
...
你不用手動填。程式會在對話累積到一定量時,把現有背景連同新對話一起交給模型:

- **還沒有背景時**——只要對方已經講出自己的基本處境,就整理成一段存起來
- **已經有背景時**——只有在他明講了根本處境的改變(換工作、畢業、實習結束)才改寫
